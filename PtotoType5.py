...
def check_to_play_sound(input):
    ...
    if input in sound_files:
        ...
        speakefor(length_in_seconds)
    else:
        print("Input not recognized.")

def speakefor(x):
    DataVariables.speaking = True
    DataVariables.speaking_end_time = pygame.time.get_ticks() + int(x * 1000)
    #check_and_stop_speaking( DataVariables.speaking_end_time)

def check_and_stop_speaking(current_time):
    if DataVariables.speaking and current_time >= DataVariables.speaking_end_time:
        DataVariables.speaking = False
        return
    else:
        check_and_stop_speaking(current_time)
def EyeGoRight():
...
def handle_input_in_arg():
    ...
    if select.select([sys.stdin], [], [], 0)[0]:
        ...
        if user_input == "left":
            ...
            start_speaking(duration=60)  # 1 sec = 60 duraction
        elif user_input == "stop speake":
            stop_speaking()  # 1 sec = 60 duraction
        else :
...
def draw_hyper_dynamic_mouth():
    ...
    if len(current_points) > 1:
        ...
        # Drawing code remains the same as original...
        # [Previous rendering code here]
